...
penguin_frw="python3 $dir/bin/package/COREPATCH/A15/framework_patch.py"
penguin_srvs="python3 $dir/bin/package/COREPATCH/A15/services_patch.py"
penguin_miui_sv="python3 $dir/bin/package/COREPATCH/A15/miui-service_patch.py"
penguin_miui_frw="python3 $dir/bin/package/COREPATCH/A15/miui-framework_patch.py"
penguin_invoke="python3 $dir/bin/package/COREPATCH/A15/invoke_patch.py"
...
JARDIR="$dir/jar_temp"
INVOKEFIX="$dir/bin/package/COREPATCH/A15/INVOKE"
...
Patch_Framework () {
    ...
    $penguin_frw true true true > /dev/null 2>&1

    target="$JARDIR/framework.jar.out/classes2.dex.out"

    $penguin_invoke > /dev/null 2>&1

    cp -rf $INVOKEFIX/android/* $target/android
...
Patch_SV () {
    ...
    $penguin_srvs true true > /dev/null 2>&1
...
Patch_MiuisvCN () {
    ...
    $penguin_miui_sv true false true true true > /dev/null 2>&1
...
Patch_MiuisvGB () {
    ...
    $penguin_miui_sv false false true true true > /dev/null 2>&1
...
Patch_MiuiFramework () {
    ...
    $penguin_miui_frw true > /dev/null 2>&1
...
if [[ $deviceTYPE == "China" ]];then
    patch "Patching Devices CN"
    Patch_MiuisvCN
else
    patch "Patching Devices Global"
    Patch_MiuisvGB
fi
