...
jar_util() 
{
    ...
    if [[ $1 == "d" ]]; then
        patch "Patching $2 : "
        if [[ -f $work_dir/build/baserom/images/system/system/framework/miui-framework.jar ]]; then
            sudo cp $work_dir/build/baserom/images/system/system/framework/miui-framework.jar $work_dir/jar_temp
        ...
        if [[ $1 == "a" ]]; then 
            if [[ -d $work_dir/jar_temp/$2.out ]]; then
                ...
                if [[ -f $work_dir/jar_temp/$2 ]]; then
                    sudo cp -rf $work_dir/jar_temp/$2 $work_dir/build/baserom/images/system/system/framework/miui-framework.jar
