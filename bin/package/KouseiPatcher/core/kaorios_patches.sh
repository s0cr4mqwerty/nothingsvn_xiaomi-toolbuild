...
create_next_smali_dir() {
    ...
    local new_smali_dir="$decompile_dir/smali_classes$new_num"

    mkdir -p "$new_smali_dir"
...
if method_start is not None:
    ...
    if registers_line:
        ...
        if not already_patched:
            # Insert Kaorios V1.0.8 block after .registers line
            for j, block_line in enumerate(reversed(kaorios_block_v107)):
                lines.insert(registers_line + 1, block_line)
            print("✓ Inserted V1.0.8 KaoriFeatureOverrides block")
            modified = True
        else:
...
if modified:
    ...
    print("✓ Successfully patched ApplicationPackageManager.smali with V1.0.8 approach")
else:
...
patch_instrumentation_new_application() {
    ...
    # Only move if it's not already in the last directory
    if [ "$current_smali_dir" != "$target_root" ]; then
        log "Relocating Instrumentation to $last_smali_dir to avoid DEX limit..."
        ...
        # Move the main class and all inner classes
        local src_dir=$(dirname "$target_file")
        mv "$src_dir"/Instrumentation*.smali "$new_dir/"

        # Update target_file to point to the new location
        target_file="$new_dir/Instrumentation.smali"
        log "✓ Relocated Instrumentation and inner classes to $last_smali_dir"
    fi
...
patch_keystore2_get_key_entry() {
    ...
    # Only move if it's not already in the last directory
    if [ "$current_smali_dir" != "$target_root" ]; then
        log "Relocating KeyStore2 to $last_smali_dir to avoid DEX limit..."

        # Create destination directory
        local rel_path="android/security"
        local new_dir="$target_root/$rel_path"
        mkdir -p "$new_dir"

        # Move the main class and all inner classes
        local src_dir=$(dirname "$target_file")
        mv "$src_dir"/KeyStore2*.smali "$new_dir/"

        # Update target_file to point to the new location
        target_file="$new_dir/KeyStore2.smali"
        log "✓ Relocated KeyStore2 and inner classes to $last_smali_dir"
    fi
...
patch_android_keystore_spi_engine_get_certificate_chain() {
    ...
    # Only move if it's not already in the last directory
    if [ "$current_smali_dir" != "$target_root" ]; then
        log "Relocating AndroidKeyStoreSpi to $last_smali_dir to avoid DEX limit..."

        # Create destination directory
        local rel_path="android/security/keystore2"
        local new_dir="$target_root/$rel_path"
        mkdir -p "$new_dir"

        # Move the main class and all inner classes
        local src_dir=$(dirname "$target_file")
        mv "$src_dir"/AndroidKeyStoreSpi*.smali "$new_dir/"

        # Update target_file to point to the new location
        target_file="$new_dir/AndroidKeyStoreSpi.smali"
        log "✓ Relocated AndroidKeyStoreSpi and inner classes to $last_smali_dir"
    fi
...
apply_kaorios_toolbox_patches() {
    ...
    create_next_smali_dir "$decompile_dir"
