...
if [[ $rom_os == "MIUI" ]]; then
mods "Update Appvault For MIUI13"
rm -rf $isSTOCKVault
mkdir -p $work_dir/build/baserom/images/product/priv-app/MIUIPersonalAssistant
cp -rf $work_dir/bin/modfile/MIUI13/appvault/MIUIPersonalAssistant/* $work_dir/build/baserom/images/product/priv-app/MIUIPersonalAssistant/
cp -rf $work_dir/bin/modfile/MIUI13/appvault/permissions/privapp_whitelist_com.miui.personalassistant.xml $work_dir/build/baserom/images/product/etc/permissions/
